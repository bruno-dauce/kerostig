...
export default async function (eleventyConfig) {
    ...
    // Revues francophones : derivees du champ `francophone` de journals.json,
    // pose par enrichissement/marquer-francophones.mjs depuis le marqueur Fr du
    // classement FNEGE. Sert au filtre "Revues francophones" de la barre
    // laterale -- la liste d'ISSN est transmise au template pour un filtrage
    // cote client (Alpine.js), cf data-issn sur chaque carte dans
    // call-card.html.
    //
    // C'etait auparavant une liste de 19 ISSN tenue a la main ici. Elle ne
    // couvrait aucune revue de rang 4 et divergeait du classement dans les deux
    // sens. Un filtre plutot qu'une donnee globale, pour la meme raison que
    // revuesCouvertesParScraper : une donnee globale ne lit pas proprement une
    // autre donnee, alors qu'un filtre recoit journals de la cascade et reste
    // juste sous --serve.
    eleventyConfig.addFilter("issnRevuesFrancophones", function (journals) {
        if (!journals) return [];
        return Object.values(journals)
            .filter((revue) => revue.francophone && revue.issn_cle)
            .map((revue) => revue.issn_cle);
    });
